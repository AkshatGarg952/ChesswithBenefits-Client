...
const Modal = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] transition-opacity duration-300">
      <div
        ref={modalRef}
        className="bg-white/95 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-orange-200 max-w-md w-full transform transition-all duration-300 scale-100 opacity-100 animate-in zoom-in-50"
      >
        {children}
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          <Mail className="w-5 h-5 text-orange-500 absolute left-3 top-1/2 transform -translate-y-1/2" />
          <input
            type="email"
            placeholder="Email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full pl-10 pr-4 py-3 bg-orange-50/50 border border-orange-200 rounded-xl focus:border-orange-400 focus:ring-2 focus:ring-orange-300 outline-none transition-all"
            required
          />
        </div>
        <div className="relative">
          <Lock className="w-5 h-5 text-orange-500 absolute left-3 top-1/2 transform -translate-y-1/2" />
          <input
            type="password"
        ...
        <button
          type="submit"
          className="w-full py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl hover:from-orange-600 hover:to-red-600 transition-all transform hover:scale-105 shadow-lg font-semibold"
        >
          Login
        </button>
      </form>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div 
          className="absolute w-96 h-96 bg-gradient-to-r from-orange-400/20 to-red-400/20 rounded-full blur-3xl animate-pulse"
          style={{
        ...
        />
        <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-gradient-to-r from-amber-400/15 to-orange-400/15 rounded-full blur-2xl animate-bounce"></div>
        <div className="absolute bottom-1/4 left-1/4 w-48 h-48 bg-gradient-to-r from-red-400/10 to-orange-400/10 rounded-full blur-xl animate-pulse"></div>
      </div>
      ...
      <nav className="relative z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-red-600 rounded-lg flex items-center justify-center transform rotate-45">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {localStorage.getItem('userId')!==undefined ? (
              <a href="/dashboard" className="text-gray-700 hover:text-orange-600 transition-colors font-medium">Dashboard</a>
            ) : (
             <button onClick={openLogin} className="text-gray-700 hover:text-orange-600 transition-colors font-medium">Dashboard</button>
            )}
          </div>

          <div className="hidden md:flex items-center space-x-4">
            <button onClick={openLogin} className="px-4 py-2 text-orange-600 hover:text-orange-700 transition-colors font-medium">
              Login
            </button>
            <button onClick={openRegister} className="px-6 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg hover:from-orange-600 hover:to-red-600 transition-all transform hover:scale-105 shadow-lg font-medium">
              Register
            </button>
          </div>
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-md border-t border-orange-200 shadow-lg">
            <div className="px-6 py-4 space-y-4">
              <a href="#" className="block text-gray-700 hover:text-orange-600 transition-colors font-medium">Home</a>
              <a href="#" className="block text-gray-700 hover:text-orange-600 transition-colors font-medium">About</a>
              <a href="#" className="block text-gray-700 hover:text-orange-600 transition-colors font-medium">Features</a>
              <a href="#" className="block text-gray-700 hover:text-orange-600 transition-colors font-medium">Dashboard</a>
              <div className="pt-4 border-t border-orange-200 space-y-2">
                <button onClick={openLogin} className="block w-full text-left text-orange-600 font-medium">Login</button>
                <button onClick={openRegister} className="block w-full px-4 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg font-medium">
                  Register
                </button>
              </div>
            </div>
                ...
                <div className="grid grid-cols-8 gap-1">
                  {Array.from({ length: 64 }).map((_, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`aspect-square flex items-center justify-center text-2xl transition-all hover:scale-110 ${
                          isLight 
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative p-8 bg-white/80 backdrop-blur-sm rounded-2xl border transition-all duration-500 hover:scale-105 ${
                  isActive ? 'border-orange-300 shadow-2xl' : 'border-orange-200 shadow-lg'
                }`}
              >
